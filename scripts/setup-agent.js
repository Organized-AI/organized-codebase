...
const { OpenRouterAdapter } = require('../src/providers/OpenRouterAdapter');
...
async function analyzeWithClaude(docs) {
  header('🤖 Analyzing your project with Claude via OpenRouter');

  const apiKey = process.env.OPENROUTER_API_KEY;
  if (!apiKey) {
    log('❌ Error: OPENROUTER_API_KEY not found in environment', 'yellow');
    log('Please set your API key in the .env file', 'yellow');
    process.exit(1);
  }

  const openrouter = new OpenRouterAdapter({ apiKey });
