...
main() {
    ...
    if check_just; then
        if [ "$just_only" = true ]; then
            ...
            echo "  just add-codex                # Add AGENTS.md + .codex/ thin adapter"
            echo "  just add-organized-router     # Add CONFIG/router/ policy layer"
            echo "  just add-pi-harness-companion # Add frontier-paired local harness scaffold"
